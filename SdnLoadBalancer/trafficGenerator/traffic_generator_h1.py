...
def generate_tcp(src_ip, dst_ip):
    send(IP(src=src_ip, dst=dst_ip) / TCP(dport=random.randint(1024, 65535), sport=random.randint(1024, 65535)))

def generate_udp(src_ip, dst_ip):
    send(IP(src=src_ip, dst=dst_ip) / UDP(dport=random.randint(1024, 65535), sport=random.randint(1024, 65535)) / Raw(load="Test payload"))
...
def send_random_packet():
    dst_ip_list = ["10.0.0.2", "10.0.0.3", "10.0.0.4"]
